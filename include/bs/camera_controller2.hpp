...
protected:
	...
	std::array<fc::Property, 18> proprs;

	bool connect();

public:
	Camera() : num_param_(18), connected_(false){};
	Camera(const Camera& o) : num_param_(18), connected_(false){};
...
inline bool Camera::connect(void)
{
	...
	if (this->checkError(mgr.GetCameraFromSerialNumber(cam_info.serialNumber, &guid)))
		return false;
	
	if (this->checkError(cam.Connect(&guid)))
		return false;
	...
	// DANGER ??
	if (this->checkError(cam.ValidateFormat7Settings(&fmt7_imset, &valid, nullptr)) || !valid)
	{
		std::cerr << std::endl << "Format7 settings are not valid" << std::endl;
		return false;
	}

	checkError(cam.SetFormat7Configuration(&fmt7_imset, 100.f));
...
class CameraController : Camera
{
	...
	bool stereo_;
	fc::Format7ImageSettings fmt7_imset_;
	...
	bool init()
	{
		...
		checkError(cam.ValidateFormat7Settings(&fmt7_imset_, &valid, nullptr));
		...
		checkError(cam.SetFormat7Configuration(&fmt7_imset_, 100.f));
		checkError(cam.StartCapture());
